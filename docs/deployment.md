...
DoubleOh ships as one container. It carries the app, the API that serves it, and the browser the Bots
drive, and it can carry its own PostgreSQL as well. It does what it does on a laptop.

```sh
docker pull ghcr.io/cheedli/doubleoh:latest

# A database you already run.
docker run -p 3001:3001 --env-file .env ghcr.io/cheedli/doubleoh:latest

# Or one inside the container. Nothing else to provision.
docker run -p 3001:3001 --env-file .env \
  -e EMBEDDED_POSTGRES=on -v doubleoh-data:/var/lib/postgresql/data ghcr.io/cheedli/doubleoh:latest
```
...
See [configuration.md](configuration.md). The short version: an administrator email and password,
`BETTER_AUTH_SECRET`, the four origin variables pointing at where people open it, `KEY_ENCRYPTION_KEY`,
`MANAGED_AGENT_TOKEN`, and an Anthropic key for the skill compiler. `docker compose up` refuses to
start on a combination that cannot work and names the variable.

**Authentication is required.** With no administrator and no identity provider configured the
deployment refuses to start, because a URL where every visitor is an administrator fails silently:
...
```sh
docker run --rm --env-file .env ghcr.io/cheedli/doubleoh:latest \
